...
import {
  ...
  CloudLightning, ChevronDown, ChevronUp, X, Info,
} from 'lucide-react';
...
export function DdnsSection({
  ...
  return (
    <div style={{ padding: 24, maxWidth: 700, fontFamily: 'var(--font-sans)' }}>
      ...
      {/* Educational Callout */}
      <DnsCallout icon={Info} title="¿Qué es el DNS dinámico?" collapsible defaultCollapsed>
        La mayoría de conexiones domésticas tienen IP dinámica: tu proveedor de internet te cambia la dirección
        periódicamente. Si tienes un dominio apuntando a tu servidor casero (por ejemplo, <em>casa.tudominio.com</em>),
        cada vez que tu IP cambie dejará de funcionar. El DNS dinámico (DDNS) detecta automáticamente estos cambios
        y actualiza tus registros DNS para que tu dominio siempre apunte a tu IP actual, sin que tengas que hacer nada.
      </DnsCallout>
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Proveedor</span>
          <InfoTooltip>Cloudflare y DuckDNS son los más populares. Cloudflare requiere tener el dominio configurado en Cloudflare; DuckDNS es gratuito y te da un subdominio .duckdns.org.</InfoTooltip>
        </div>
        ...
        <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: 4 }}>
          Hostname
          <InfoTooltip>El subdominio o dominio cuyo registro A quieres mantener actualizado con tu IP pública. Ejemplo: casa.tudominio.com</InfoTooltip>
        </label>
        <input
          value={hostname}
        ...
        {type === 'cloudflare' && (
          <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <InfoTooltip>Oculta tu IP real detrás de Cloudflare. Tu tráfico pasará por sus servidores, añadiendo protección DDoS y caché. Si solo necesitas acceso directo (VPN, SSH, etc.), déjalo desactivado.</InfoTooltip>
          </label>
        )}
