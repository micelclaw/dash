...
interface SubdomainSectionProps {
  request: MicelclawSubdomainData | null;
  loading: boolean;
...
export function MicelclawSubdomain({ request, loading, onRequest, onRelease, onCheck }: SubdomainSectionProps) {
  if (loading) {
    return (
      <div style={{
...
function SubdomainRequestForm({ onRequest }: { onRequest: (subdomain: string, recordTypes: string[]) => Promise<void> }) {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 600 }}>
      ...
      {/* Callout educativo */}
      <DnsCallout icon={Info}>
        Tu subdominio es como alquilar un apartamento en el edificio Micelclaw.
        Obtienes <strong style={{ color: 'var(--text-dim)' }}>tunombre.micelclaw.com</strong> y
        Micelclaw configura todo el DNS automáticamente. Puedes usarlo para acceder
        a tu servidor desde cualquier lugar del mundo.
      </DnsCallout>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{
            ...
            Subdomain
            <InfoTooltip>Elige un nombre corto y memorable. Solo letras, números y guiones. Ejemplo: 'paco', 'casa-paco', 'servidor1'.</InfoTooltip>
          </div>
          <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {RECORD_TYPE_OPTIONS.map(opt => (
              ...
              >
                ...
                />
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text)' }}>
                    {opt.label}
                    <InfoTooltip>{opt.tooltip}</InfoTooltip>
                  </div>
...
function SubdomainStatus({ request, onRelease, onCheck }: {
  request: MicelclawSubdomainData;
  onRelease: () => Promise<void>;
  onCheck: () => Promise<void>;
}) {
    ...
    <div style={{ padding: 24, maxWidth: 600 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', fontSize: '0.75rem' }}>
          {request.record_types && <Detail label="Record Types" value={request.record_types.join(', ')} />}
          {request.server_ip && <Detail label="Server IP" value={request.server_ip} />}
          {request.requested_at && <Detail label="Requested" value={new Date(request.requested_at).toLocaleDateString()} />}
