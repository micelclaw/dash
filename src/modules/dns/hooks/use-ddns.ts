...
export function useDdns() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await api.get<{ data: DdnsStatus }>('/dns/ddns/status');
      setStatus(res.data);
    } catch {
      // DDNS not available
    }
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await api.get<{ data: DdnsHistoryEntry[] }>('/dns/ddns/log');
      setHistory(res.data);
    } catch {
      // ignore
    }
  ...
  const forceUpdate = useCallback(async () => {
    setUpdating(true);
    try {
      const res = await api.post<{ data: DdnsUpdateResult[] }>('/dns/ddns/check-now');
