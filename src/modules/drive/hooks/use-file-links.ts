...
/**
 * Linked records of a file for the Drive inspector's Related panel
 * (same shape `use-note-links` produces for the notes editor).
 */
export function useFileLinks(fileId: string | null) {
    ...
    api.get<ApiListResponse<EntityLink>>('/links', {
      ...
      .then(async (res) => {
        ...
        const resolved = res.data
          .map(link => resolveLink(link, fileId))
          // graph_entity links are shown in GraphProximityPanel
