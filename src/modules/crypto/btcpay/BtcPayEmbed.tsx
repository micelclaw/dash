...
export function Component() {
  ...
  const checkStatus = useCallback(async () => {
    try {
      ...
      const svc = (res.data as any).services?.find((s: any) => s.name === 'btcpay');
    ...
    return (
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{
          ...
          <span style={{ fontWeight: 500, color: 'var(--text)' }}>BTCPay Server</span>
          <span style={{ color: 'var(--text-muted)' }}>Payment Processor</span>
          <div style={{ flex: 1 }} />
          {status?.ram_mb != null && (
        ...
        <iframe src="http://localhost:3003" title="BTCPay Server" style={{ flex: 1, border: 'none', background: '#fff' }} />
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      {phase === 'loading' && (<><Loader2 size={32} className="spin" /><span>Checking BTCPay Server...</span></>)}
      {phase === 'installing' && (<><Loader2 size={32} className="spin" style={{ color: 'var(--amber)' }} /><span>Starting BTCPay Server...</span><span style={{ fontSize: 12, color: 'var(--text-muted)' }}>This may take a moment on first launch</span></>)}
