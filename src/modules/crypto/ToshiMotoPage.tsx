...
export function Component() {
  ...
  const checkStatus = useCallback(async () => {
    try {
      ...
      const svc = (res.data as any).services?.find((s: any) => s.name === 'toshi-moto');
    ...
    return (
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{
          ...
          <span style={{ fontWeight: 500, color: 'var(--text)' }}>Toshi Moto</span>
          <span style={{ color: 'var(--text-muted)' }}>Bitcoin Explorer</span>
          <div style={{ flex: 1 }} />
          {status?.ram_mb != null && (
        ...
        <iframe src="http://localhost:3100" title="Toshi Moto" style={{ flex: 1, border: 'none', background: '#fff' }} />
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      {phase === 'loading' && (<><Loader2 size={32} className="spin" /><span>Checking Toshi Moto...</span></>)}
      {phase === 'installing' && (<><Loader2 size={32} className="spin" style={{ color: 'var(--amber)' }} /><span>Starting Toshi Moto...</span><span style={{ fontSize: 12, color: 'var(--text-muted)' }}>MongoDB + Bitcoin explorer</span></>)}
      {phase === 'error' && (<><AlertTriangle size={32} style={{ color: 'var(--error)' }} /><span>Error</span>{error && <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{error}</span>}<button onClick={() => { setError(null); startService(); }} className="crypto-btn">Retry</button></>)}
