...
import { X, Loader2, CheckCircle, AlertTriangle, Bitcoin } from 'lucide-react';
...
export function ToshiMotoInstallPanel({ btcRunning, onClose, onDone }: Props) {
  ...
  const handleInstall = async () => {
    ...
    setError(null);
    try {
      await api.post('/crypto/toshi-moto/start');
      setStep('done');
    } catch (err: any) {
      setError(err?.message || 'Failed to install Toshi Moto');
      setStep('error');
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.9375rem', fontWeight: 600, color: 'var(--text)' }}>
            ...
            {step === 'info' && 'Install Toshi Moto'}
            {step === 'installing' && 'Installing Toshi Moto'}
            {step === 'done' && 'Toshi Moto Ready'}
            {step === 'error' && 'Error'}
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: 16, overflow: 'auto', flex: 1 }}>
          ...
          {step === 'info' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ fontSize: '0.8125rem', color: 'var(--text-dim)', lineHeight: 1.6 }}>
                Bitcoin explorer with Sankey flow diagrams for transaction visualization. Watch-only — no private keys stored.
              </div>
              ...
              <div>
                ...
                <div className="wizard-dep-row">
                  <span className="wizard-dep-dot" style={{ background: 'var(--text-muted)' }} />
                  <span>MongoDB</span>
                  <span className="wizard-dep-label">Auto-installed</span>
                </div>
              </div>
              ...
              {!canInstall && (
                <div className="wizard-warning">
                  ...
                  <span>Bitcoin Core must be running before installing Toshi Moto.</span>
                </div>
              )}

              <div className="wizard-info">
                This will also install a MongoDB instance for Toshi Moto's data.
              </div>
          ...
          {step === 'installing' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '24px 0' }}>
              <Loader2 size={32} style={{ color: '#f7931a', animation: 'spin 1s linear infinite' }} />
              <div style={{ fontSize: '0.875rem', color: 'var(--text-dim)' }}>
                Installing Toshi Moto...
              </div>
              <div style={{ width: '100%', marginTop: 8 }}>
                <CryptoLogs service="toshi-moto" active tail={8} />
              </div>
            </div>
          )}
          ...
          {step === 'done' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '40px 0' }}>
              ...
              <div style={{ fontSize: '0.875rem', color: 'var(--text)' }}>
                Toshi Moto installed successfully
              </div>
